...
struct TracksView: View {
    ...
    var body: some View {
        ...
        ZStack {
                    ...
                    HStack {
                        ...
                        Button(action: {
                            if !searchTerm.isEmpty {
                                searching = true
                                tracksViewModel.send(.fetch(searchTerm))
                            }
                        }, label: {
                            Text("Search")
                        })
            ...
            if !tracksViewModel.errorMessage.isEmpty {
                Text(tracksViewModel.errorMessage)
                    .font(.title)
                    .foregroundColor(.red)
            }

            if tracksViewModel.noResults {
                Text("No Results Found")
                    .font(.title)
            }

            if !tracksViewModel.tracks.isEmpty && tracksViewModel.errorMessage.isEmpty {
                VStack {
                    HStack {
                        Spacer()
                        Button(action: {
